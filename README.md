...
详见仓库根目录的 `CHANGELOG.md`。

### F0.0.1
- 支持多实例端口运行（`python app.py --port xxxx`）
- `start.bat` 支持输入端口，并在端口占用时自动寻找空闲端口
- 启动后自动打开对应浏览器地址
...
如果你只是双击 `start.bat`，脚本现在也会**提示你输入端口**；
- 你手动输入端口，就按你输入的启动
- 你直接回车，它会从 **5000** 开始自动检测空闲端口，如果被占用就递增尝试 `5001 / 5002 / ...`
